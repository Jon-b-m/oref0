...
//console.error(glucose_data[i].glucose, bgTime, Math.round(hoursAfterMeal*100)/100, bucketed_data[bucketed_data.length-1].display_time);
    ...
    for (i=0; i < bucketed_data.length-3; ++i) {
        ...
        if(bgTime > mealTime) {
            ...
            if (profile.use_Static_COB_decay == true) {
                var absorbed = ci * profile.carb_ratio * profile.cobDecayAdjust / profile.sens ; //calculate absorbed carbs from profile.sens instead of sens (= profile.sens / sensitivityRatio) to reduce COB decay at elevated BGs when using dynamic ISF. This will make the carb absorption model static instead of dynamic. cobDecayAdjust: adjustment factor for carb absorption / COB decay. Higher number = faster absorption ;
                console.log("Enable Static COB decay = " + profile.use_Static_COB_decay + ", COB decay adjustment factor: " + profile.cobDecayAdjust + " CR: " + profile.carb_ratio + " ISF: " + profile.sens + " CSF: " + csf + ". ");
                } else {
                    var absorbed = ci * profile.carb_ratio / sens ;
                    console.log("Enable Static COB decay = " + profile.use_Static_COB_decay + ". ");
            }
