...
//console.error(glucose_data[i].glucose, bgTime, Math.round(hoursAfterMeal*100)/100, bucketed_data[bucketed_data.length-1].display_time);
    ...
    for (i=0; i < bucketed_data.length-3; ++i) {
        ...
        if(bgTime > mealTime) {
            ...
            if (profile.enableChris == true) {
                var absorbed = ci * profile.carb_ratio / profile.sens ; //calculate absorbed carbs from profile.sens instead of sens (= profile.sens / sensitivityRatio) to reduce COB decay at elevated BGs when using dynamic ISF. This will make the carb absorption model static instead of dynamic.
