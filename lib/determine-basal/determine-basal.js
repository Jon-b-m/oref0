    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        if (bg_ISF < 1) {                                                                                                     
            ...
            if ( liftISF < profile.autoisf_min ) {                                                                              
                isfadaptionreason = "final ISF factor " + round(liftISF,2) + " limited by autoisf_min " + profile.autoisf_min;  
                console.error(isfadaptionreason);
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, pumphistory, preferences, basalprofile, tdd, tdd_averages, currentTime) { 
    ...
    if (weightedAverage > 0 && average_7days > 0) {
        tddReason = ", TDD: " + round(tdd,1) + " U, Weighted avg: " + round(weightedAverage,1) + " U, Total data avg: " + round(average_7days,1) + " U";

        } else { tddReason = ", TDD: " + round(tdd,2); }
    // -------------------- END OF TDD ----------------------------------------------------
    ...
    if (average_7days > 0) {
        tdd24h_7d_Ratio = weightedAverage / average_7days;
    }
    ...
    if (tdd24h_7d_Ratio > 1) {
        ...
        console.log("Basal adjustment with a 24 hour to 7 day average TDD ratio limited by Autosens max setting. Ratio " + tdd24h_7d_Ratio + ". Upper limit = Autosens max (" + profile.autosens_max + ")");
    }
    else if (tdd24h_7d_Ratio < 1) {
        tdd24h_7d_Ratio = Math.max(tdd24h_7d_Ratio, profile.autosens_min);
        tdd24h_7d_Ratio = round(tdd24h_7d_Ratio,2);
        console.log("Basal adjustment with a 24 hour to 7 day average TDD ratio limited by Autosens min setting. Ratio: " + tdd24h_7d_Ratio + ". Lower limit = Autosens min (" + profile.autosens_min + ")");
    } 
    else { 
        console.log("Basal adjusted with a 24 hour to 7 day average TDD ratio: " + tdd24h_7d_Ratio);
    }

    var basal_ratio_log = ", Basal ratio: " + tdd24h_7d_Ratio;
    ...
    if (chrisFormula == true && tdd > 0) {
        ...
        if (newRatio > maxLimitChris) {
            ...
            log = ", Dynamic ISF limjted by autosens_min setting: " + minLimitChris + " (" +  round(newRatio,2) + "). ";
    ...
    if (chrisFormula == false && useDynamicCR == false) {
        tddReason += "";
    } else { tddReason += log_isfCR + formula + limitLog + basal_ratio_log; }
...
//    var sensitivityRatio;
    ...
    if (sensitivityRatio && profile.enableChris == false) { // Disable adjustment of basal by sensitivityRatio when using dISF
        ...
        basal = round_basal(basal, profile);
        process.stderr.write("TDD-adjustment of basals activated, using tdd24h_7d_Ratio " + round(tdd24h_7d_Ratio,2) + ", TDD 24h = " + round(tdd_before,2) + "U, Weighted average TDD = " + round(weightedAverage,2) + "U, (Weight percentage = " + weightPercentage + "), 7-day average TDD = " + round(average_7days,2) + "U. " );
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            //IOBpredBG = IOBpredBGs[IOBpredBGs.length-1] + predBGI + predDev; // Adding dynamic ISF in predictions for UAM, ZT and IOB:
             IOBpredBG = IOBpredBGs[IOBpredBGs.length-1] + (round(( -iobTick.activity * (1800 / ( tdd * (Math.log((Math.max( IOBpredBGs[IOBpredBGs.length-1],39) / insulinFactor ) + 1 ) ) )) * 5 ),2)) + predDev;
            // calculate predBGs with long zero temp without deviations
            //var ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + predZTBGI; // Adding dynamic ISF in predictions for UAM, ZT and IOB:
            var ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + (round(( -iobTick.iobWithZeroTemp.activity * (1800 / ( tdd * (Math.log(( Math.max(ZTpredBGs[ZTpredBGs.length-1],39) / insulinFactor ) + 1 ) ) )) * 5 ), 2));
            ...
            //UAMpredBG = UAMpredBGs[UAMpredBGs.length-1] + predBGI + Math.min(0, predDev) + predUCI; // Adding dynamic ISF in predictions for UAM, ZT and IOB:
            UAMpredBG = UAMpredBGs[UAMpredBGs.length-1] + (round(( -iobTick.activity * (1800 / ( tdd * (Math.log(( Math.max(UAMpredBGs[UAMpredBGs.length-1],39) / insulinFactor ) + 1 ) ) )) * 5 ),2)) + Math.min(0, predDev) + predUCI;
...
// Print summary variables based on predBGs etc.
    ...
    rT.reason = isfreason + ", COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
    if (lastCOBpredBG > 0) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if (typeof profile.maxSMBBasalMinutes === 'undefined' ) {
                ...
                if( insulinReq > maxBolus ) {
                  console.error("SMB limited by maxBolus: " + maxBolus + " ( " + insulinReq + " U)");a
                }
                ...
                if( insulinReq > maxBolus ) {
                  console.error("SMB limited by maxUAMSMBBasalMinutes [ " + profile.maxUAMSMBBasalMinutes + "m ]: " + maxBolus + "U ( " + insulinReq + "U )");
                ...
                if( insulinReq > maxBolus ) {
                  console.error("SMB limited by maxSMBBasalMinutes: " + profile.maxSMBBasalMinutes + "m ]: " + maxBolus + "U ( insulinReq: " + insulinReq + "U )");
